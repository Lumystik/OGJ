...
// El id_test y id_pregunta puede venir por POST o por GET, se consideran ambos casos.
...
if( isset($id_pregunta) ) {
    $q = "select pregunta from equipo01_preguntastest where id_pregunta = $id_pregunta";
    $rs = ejecutar($q);
...
if( isset($_POST['eliminar']) ) {
    ...
    $q = "delete from equipo01_opciones_pregunta where id_opcion = $id_opcion";
    $rs = ejecutar($q);
}
...
if( isset($_POST['insertopcion']) ) {
    $opcion = $_POST['opcion'];
    $id_pregunta = $_POST['id_pregunta'];
	$q = "insert into equipo01_opciones_pregunta(opcion, id_pregunta) values ('$opcion','$id_pregunta')";
	$rs = ejecutar($q);
}

function crea_lista_opciones($id_pregunta, $id_test) {
    $q = "select * from equipo01_opciones_pregunta where id_pregunta = $id_pregunta";
	$rs = ejecutar($q);
	$contenido = "";
...
<body>
	...
	<p id="observatorio"></p>
	<div class="sidenav">
		<a href="bak_inicio.php">INICIO</a> <a href="bak_foros.php">FOROS</a> <a href="testp.php" style="background-color: white;color: #2E2E2C !important;">TEST</a> <a href="cuestionariosbackend.php">CUESTIONARIOS</a> <a href="#">MAPAS</a> <a href="#">ARTÍCULOS</a>
...
<button style="background-color: white;    color: #5b5b5c;position:relative;left:20%;margin-top:10px" class="btn"  data-toggle="modal" data-target="#myModal10">NUEVA OPCION<span style="background-color:#00A99D; 
  ...
  <br>
  <?php crea_lista_opciones($id_pregunta, $id_test); ?>
  
  <br>
  <a href="admin_preguntas.php?id_test=<?php echo $id_test;?>">Regresar a Administrador de Preguntas</a>
			</div>
...
<form method="post" action="admin_opciones.php" >
    ...
    <input type="hidden" name="id_test" value="<?php echo $id_test; ?>">
     ...
     <br><br><br>
  	<br><input class="btn" type="submit" value="Guardar opción">
  </form>   
