...
// El id de cuestionario puede venir por POST o por GET, se consideran ambos casos.
...
if( isset($id_cuestionario) ) {
    $q = "select titulo from equipo01_cuestionario where id_cuestionario = $id_cuestionario";    
    $rs = ejecutar($q);
...
if( isset($_POST['insertpreguntacuest']) ) {
    $pregunta = $_POST['pregunta'];
    $id_cuestionario = $_POST['id_cuestionario'];
	$q = "insert into equipo01_preguntascuestionario (pregunta, id_cuestionario) values ('$pregunta', '$id_cuestionario')";
	$rs = ejecutar($q);
}
...
if( isset($_POST['eliminar']) ) {
    ...
    $q = "delete from equipo01_opciones_preguntacues where id_pregunta = $id_pregunta";
    $rs = ejecutar($q);

    // Se elimina la pregunta.
    $q = "delete from equipo01_preguntascuestionario where id_pregunta = $id_pregunta";
    $rs = ejecutar($q);
}

function crea_lista_preguntas($id_cuestionario) {
    $q = "select * from equipo01_preguntascuestionario where id_cuestionario = $id_cuestionario";
	$rs = ejecutar($q);
	$contenido = "";
	while($r = mysqli_fetch_array($rs)) {
		...
		$contenido .= "
            <tr>
                ...
                <td>$pregunta</td>
                <td>
                    <form action='admin_preguntascuest.php' method='post'>
                ...
                <td>
                	<button class='btn ver'><a href='admin_opcionescuest.php?id_pregunta=$id_pregunta&id_cuestionario=$id_cuestionario'>Agregar/Quitar</a></button>
                </td>
            </tr>
        ";
	}
...
<body>
	...
	<p id="observatorio"></p>
	<div class="sidenav">
		<a href="bak_inicio.php">INICIO</a> <a href="bak_foros.php">FOROS</a> <a href="testp.php" >TEST</a> <a href="cuestionariosbackend.php" style="background-color: white;color: #2E2E2C;">CUESTIONARIOS</a> <a href="#">MAPAS</a> <a href="back_articulos_bueno.php">ARTÍCULOS</a>
...
<button style="background-color: white;    color: #5b5b5c;position:relative;left:20%;margin-top:10px" class="btn"  data-toggle="modal" data-target="#myModal10">NUEVA PREGUNTA<span style="background-color:#00A99D; 
			...
			<div>
			    ...
			    <?php crea_lista_preguntas($id_cuestionario); ?>
    
                <br>
			<a href="cuestionariosbackend.php" style="color:white !important;margin-top:20px;">Regresar al administrador de cuestionarios</a>
			</div>
...
<p>AGREGA UNA NUEVA PREGUNTA</p>
    <form method="post" action="admin_preguntascuest.php" >
        <input type="hidden" name="insertpreguntacuest" value="insertpreguntacuest">
        <input type="hidden" name="id_cuestionario" value="<?php echo $id_cuestionario; ?>">
        Pregunta nueva:<br>
        <input type="text" name="pregunta">
        <br><br><br><input class="btn enviar" type="submit" value="Guardar pregunta">
    </form>
      </div>
    </div>
	  </div></div>
